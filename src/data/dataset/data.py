...
def load_data(conf: config.Config) -> torch.utils.data.Dataset:
    """Load a HuggingFace dataset using the configuration values."""
    train_files = None
    ...
    return datasets.load_dataset(
        conf.data_configs.dataset.name,
        data_files=train_files,
        split=conf.data_configs.dataset.split,
        num_proc=conf.data_configs.dataset.load_num_proc,
    )
